...
**`gdal_calc.py` e `gdaldem` NÃO estavam nessa lista.** Os dois são necessários.

✅ **Conferido no servidor em 25/08/2026: os dois existem** (`/usr/bin/gdaldem`,
`/usr/bin/gdal_calc.py`), GDAL 3.8.4, com `osgeo.gdal` 3.8.4 e numpy 1.26.4. O risco
R-02 está eliminado e o Plano B abaixo não precisou ser usado. A lista do doctor foi
atualizada mesmo assim, para o dia em que o ambiente mudar.
...
- ⚠️ **`-cutline_srs` não existe no GDAL 3.8.4** (medido: `gdalwarp --help` lista só
  `-cutline`, `-cl`, `-cwhere`, `-csql`, `-cblend`, `-crop_to_cutline`). O GDAL lê o
  CRS do próprio arquivo de corte — o GeoJSON é WGS84 por especificação, e SIRGAS
  2000 fica a menos de 1 m dele.
...
Onde `$QA_MASK` = `NDVI_QA_MASK_BITS` (**59** para L4/5/7, **63** para L8/9 — o bit 2
é cirrus).
