...
function resolveDocPathFromSegments(segments: string[]): string | null {
  ...
  const allowed = new Set(["conversations", "simcar_clips", "cbers_wpm_jobs", "landsat_jobs", "vertices_jobs", "processing_jobs", "containment_jobs", "geometry_errors_jobs", "processar_projeto_jobs", "simcar_oraculo_jobs", "simcar_lotes_jobs", "receipts", "croqui_jobs", "ndvi_scene_jobs"]);
...
export function saveUserBuffer(args: {
  ...
  area:
    ...
    | "processar-projeto/output"
    | "processar-projeto/import-pdf"
    | "simcar-oraculo/input"
    | "simcar-oraculo/output"
    | "simcar-oraculo/import-pdf"
    | "simcar-oraculo/process-pdf"
