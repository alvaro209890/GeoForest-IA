...
export function ndviSceneArchiveSubdir(pathId: string, row: string, year: string | number): string {
  return path.join(
...
function recordPath(archiveId: string): string {
  return path.join(SCENE_INDEX_DIR, `${safeSegment(archiveId, "ndvi-scene")}.json`);
}

export function saveNdviSceneArchiveRecord(record: NdviSceneArchiveRecord): void {
  writeJsonAtomic(recordPath(record.archiveId), record);
}

export function readNdviSceneArchiveRecord(archiveId: string): NdviSceneArchiveRecord | null {
  try {
    const arquivo = recordPath(archiveId);
    if (!fs.existsSync(arquivo)) return null;
    return JSON.parse(fs.readFileSync(arquivo, "utf8")) as NdviSceneArchiveRecord;
  } catch {
    return null;
  }
}
...
export function listNdviSceneArchiveRecords(): NdviSceneArchiveRecord[] {
  try {
    ...
    return fs
      ...
      .map((n) => {
        try {
          return JSON.parse(fs.readFileSync(path.join(SCENE_INDEX_DIR, n), "utf8")) as NdviSceneArchiveRecord;
        } catch {
          return null;
        }
      })
...
export function markNdviSceneArchiveUserDeleted(uid: string, jobId: string): void {
  ...
  for (const record of listNdviSceneArchiveRecords()) {
    ...
    saveNdviSceneArchiveRecord({ ...record, userDeletedAt: agora, updatedAt: agora });
  }
}
