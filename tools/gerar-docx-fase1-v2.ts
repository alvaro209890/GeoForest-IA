// Gera o DOCX timbrado IMAP da Fase 1 v2 a partir do JSON salvo pela tool
// testar-fase1-v2-imovel-real.ts. Mesmo caminho de código da rota HTTP:
// buildSimcarReportDocxBuffer com auasMeta + auasText.
import fs from "node:fs";

const meta = JSON.parse(fs.readFileSync("/tmp/f1_v2_result.json", "utf-8"));

const { buildSimcarReportDocxBuffer } = await import("../backend/simcar/report-docx.ts");
...
fs.writeFileSync("/tmp/Laudo_Fase1_v2_imovel_real.docx", docx);
console.log(`DOCX salvo: /tmp/Laudo_Fase1_v2_imovel_real.docx (${(docx.length / 1024 / 1024).toFixed(2)} MB)`);
