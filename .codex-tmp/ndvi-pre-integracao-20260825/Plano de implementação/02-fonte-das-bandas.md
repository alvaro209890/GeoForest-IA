...
`prepareDownloadableLandsatItem(item)` em `backend/landsat/stac-search.ts` **assina todos
os assets** do item do Planetary Computer — então os três saem assinados de graça, sem
código novo de autenticação.
...
Máscara padrão sugerida: bits 0+1+3+4+5 = **27**; mais o bit 2 (cirrus, +4 = **31**)
